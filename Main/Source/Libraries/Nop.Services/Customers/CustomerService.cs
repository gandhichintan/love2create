...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        private readonly IRepository<CustomerMisc> _customerMiscRepository;
        private readonly IRepository<RetailerProfile> _retailerProfileRepository;
        ...
        public CustomerService(ICacheManager cacheManager,
            ...
            IRepository<CustomerMisc> customerMiscRepository, 
            IRepository<RetailerProfile> retailerProfileRepository,
            IRepository<Author> authorRepository)
        {
            ...
            this._customerMiscRepository = customerMiscRepository;
            this._retailerProfileRepository = retailerProfileRepository;
        ...
        public virtual IList<Consumer> GetAllConsumers()
        {
            var query = from consumers in _consumerRepository.Table
        ...
        public virtual void InsertCustomerMisc(CustomerMisc customerMisc)
        {
            if (customerMisc == null)
                throw new ArgumentNullException("customerMisc");

            _customerMiscRepository.Insert(customerMisc);
        }

        public virtual void UpdateCustomerMisc(CustomerMisc customerMisc)
        {
            if (customerMisc == null)
                throw new ArgumentNullException("customerMisc");

            _customerMiscRepository.Update(customerMisc);

            //event notification
            _eventPublisher.EntityUpdated(customerMisc);
        }

        public virtual CustomerMisc GetCustomerMiscByCustomerId(int customerId)
        {
            var query = from cm in _customerMiscRepository.Table
        ...
        public virtual void InsertRetailerProfile(RetailerProfile retailerProfile)
        {
            if (retailerProfile == null)
                throw new ArgumentNullException("retailerProfile");

            _retailerProfileRepository.Insert(retailerProfile);
        }
        ...
        public virtual IPagedList<Author> GetAllAuthors(string AuthorName = "", int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            if (!String.IsNullOrEmpty(AuthorName))
                query = query.Where(c => c.Title.Contains(AuthorName));
            query = query.OrderByDescending(c => c.CreatedOn).ThenByDescending(c => c.Id);

            //paging
            return new PagedList<Author>(query.ToList(), pageIndex, pageSize);
        }
